...
- The executor also honors three environment overrides (see `tools/docker/src/executors/version-tags.ts`): `PORTFOLIO_IMAGE_TAG` overrides the configured `versionTags` (comma-separated) so CI can stamp a release version or the `staging` tag without editing `project.json`; `PORTFOLIO_BUILDER_TAG` picks which `builder`/`local-http-server` base image tag to build FROM (falls back to the Dockerfile default `latest`); `MFE_BASE_URL` is passed through as a build arg and is consumed only by the shell (see below).
- **Two environments, both on the single k3s cluster / one MetalLB IP, routed by the reverse proxy on distinct hostnames:** production (`ichirokuxvi.com` + `mfe.ichirokuxvi.com`) and staging (`staging.ichirokuxvi.com` + `mfe.staging.ichirokuxvi.com`). They are one Helm release in one namespace; staging apps are gated by `staging.enabled` in `values.yaml`.
...
- `.github/workflows/docker-ci.yml` (**staging, on push to `main`**) computes affected projects against the last successful run, builds/tests them, then builds affected micro-frontends with `PORTFOLIO_IMAGE_TAG=staging` and the staging `MFE_BASE_URL`, `helm upgrade`s (preserving the live production version from `/root/helm-live/prod-tag.yaml`), and `kubectl rollout restart`s the changed `*-staging` deployments so they re-pull the mutable tag.
- `.github/workflows/release.yml` (**production, on GitHub Release published**) builds *all* micro-frontends at the release commit with `PORTFOLIO_IMAGE_TAG=<version>,latest` and the production `MFE_BASE_URL`, then runs `k8s/helm/deploy-release.sh <version>` on the host, which pins `productionImageTag` in `/root/helm-live/prod-tag.yaml` and `helm upgrade`s. Production apps are pinned to immutable version tags; rollback is `deploy-release.sh <older-version>` (or `helm rollback`).
