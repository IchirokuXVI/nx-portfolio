 ...
 *
 * The same layout as the suggestions decider, keyed on the product rather than
 * on a queue entry. A killed run resumes for free because everything a
 * subcommand needs is on disk between invocations. There are two files and
 * nothing else:
 *
 * - `state.json`: the run's identity, the two urls, the listing cursor and the
 *   ids already decided.
 * - `decisions.jsonl`: a header line, then one line per decided product.
 *
 ...
 * the append and the rewrite, so `loadRun` reconciles the two and the JSONL
 * wins. A product already in the file is never asked about again.
 */
...
export function createRun(
  dir,
  { runId, mainUrl, rehearsalUrl, mainUser, model, total }
) {
  ...
  const state = {
    ...
    total: total ?? 0,
    // One listing, not one per chain: a product belongs to no chain, so the
    // walk is a single cursor over the ungrouped products.
    cursor: null,
    exhausted: false,
    decidedIds: [],
    createdRefs: {},
 ...
 *
 * The JSONL is the record of truth for what was decided, because it is appended
 * before the state is rewritten.
 */
export function loadRun(dir) {
  ...
  for (const record of records) {
    decidedIds.add(record.itemId);
    if (record.ref && record.rehearsalGroupId) {
      createdRefs[record.ref] = record.rehearsalGroupId;
    }
  }
...
/** Appends one decided product and rewrites the state in that order. */
