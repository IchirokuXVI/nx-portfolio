...
describe('list contracts', () => {
  it('pins the enum wire values', () => {
    ...
    expect(SettlementOutcome.BOUGHT).toBe('BOUGHT');
    expect(SettlementOutcome.NOT_AVAILABLE).toBe('NOT_AVAILABLE');
  });
  ...
  it('pins the message subjects', () => {
    ...
    expect(LINE_PATTERNS.settle).toBe('line.settle');
    expect(LINE_PATTERNS.settlements).toBe('line.settlements');
    expect(LINE_PATTERNS.itemSettlements).toBe('line.itemSettlements');
  ...
  it('pins the quantity bounds, which the gateway DTOs and core now share', () => {
    ...
    // Zero since plan 0047: a line at zero is stocked rather than deleted, and
    // it is where the reel on the list page puts one.
    expect(LINE_QUANTITY_MIN).toBe(0);
  ...
  it('pins the realtime event subjects', () => {
    ...
    expect(RealtimeEvent.LineSettled).toBe('line.settled');
