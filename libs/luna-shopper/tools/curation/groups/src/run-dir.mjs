 ...
 *
 * The same layout as `curation-suggestions`, with one difference: the walk is
 * one cursor rather than one per chain, because ungrouped products are a single
 * list and not a queue per supermarket.
 *
 ...
 *
 * - `state.json`: the run's identity, the two urls and the walk cursor.
...
export function createRun(
  ...
  const state = {
    ...
    limit,
    // The walk: the cursor of the page `next` is standing on, and whether the
    // list has run out.
    cursor: null,
    exhausted: false,
    decidedIds: [],
