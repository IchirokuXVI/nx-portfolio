...
/**
 * A run directory whose state already holds `e1`, so a `decide` that reaches
 * the command says which row it was given, and says it before any gateway is
 * opened.
 */
function decidedRunDir() {
  const dir = mkdtempSync(join(tmpdir(), 'curation-row-'));
  writeFileSync(
    join(dir, 'state.json'),
    JSON.stringify({ runId: 'r1', mainUrl: 'http://x', decidedIds: ['e1'] })
  );
...
test('decide takes the row as --row, which is what the orchestrator sends', async () => {
  await assert.rejects(
    () =>
      run(['decide', '--run-dir', decidedRunDir(), '--row', 'e1'], {
        stdin: () => '{}',
      }),
    /Entry e1 is already in/
  );
});

test('--entry is still the same flag under its older name', async () => {
  await assert.rejects(
    () =>
      run(['decide', '--run-dir', decidedRunDir(), '--entry', 'e1'], {
        stdin: () => '{}',
      }),
    /Entry e1 is already in/
  );
