...
type CheckFunction<DataType, FilterType> = (
  ...
  filterConfig: WithRequired<
    FormattedFilterConfig<DataType, FilterType>,
    'dataField'
  >
) => boolean;
...
export class InMemoryFilter<DataType, FilterType> {
  ...
  readonly checks = {
    equals: (itemValue, filterValue, filterConfig) =>
      filterConfig.dataField.every(
        (dataField) => findField(itemValue, dataField) == filterValue
      ),
    strictEquals: (itemValue, filterValue, filterConfig) =>
      filterConfig.dataField.every(
        (dataField) => findField(itemValue, dataField) === filterValue
      ),
    valueIncludes: (itemValue, filterValue, filterConfig) =>
      filterConfig.dataField.every((dataField) => {
        const ogFieldValue = findField(itemValue, dataField);

        const fieldValue = Array.isArray(ogFieldValue)
          ? ogFieldValue
          : [ogFieldValue];

        return fieldValue.includes(filterValue);
      }),
    filterIncludes: (itemValue, filterValue, filterConfig) =>
      filterConfig.dataField.every((dataField) => {
        const itemFieldValue = findField(itemValue, dataField);

        return (Array.isArray(filterValue) ? filterValue : [filterValue]).every(
          (filterVal) => filterVal.includes(itemFieldValue)
        );
      }),
    filterIncludesAny: (itemValue, filterValue, filterConfig) =>
      filterConfig.dataField.every((dataField) => {
        const itemFieldValue = findField(itemValue, dataField);
    ...
    deepEquals: (itemValue, filterValue, filterConfig) =>
      filterConfig.dataField.every((dataField) =>
        deepEqual(findField(itemValue, dataField), filterValue, {
          strict: true,
        })
      ),
  ...
  applyFilter(
    ...
    const filteredItems = items.filter((item) => {
      for (const key of Object.keys(filter) as (keyof FilterType)[]) {
        ...
        const dataField = (
          Array.isArray(filterConfig.dataField) && filterConfig.dataField.length
            ? filterConfig.dataField
            : [filterConfig.dataField || key]
        ) as RecursivePaths<DataType>[];

        if (!dataField)
          throw new Error(
            `dataField wasn't found for filter config ${String(key)}`
          );

        if (
          !filterConfig.check(item, filterValue, { ...filterConfig, dataField })
        ) {
          return false;
        }
      }
    ...
    const sortedItems = options?.sort
      ? filteredItems.sort(multiSort<DataType>(options.sort))
      : filteredItems;
