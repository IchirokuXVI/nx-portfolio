...
export function runSharedTreatmentServiceTests(
  ...
  describe("Shared ToothTreatmentServiceI behavior", () => {
    ...
    const mockToothTreatment: Optional<ToothTreatment, 'id'> = { teeth: ["11"], groupTeeth: false, status: ToothTreatmentStatus.COMPLETED, type: TreatmentType.STANDARD, zones: [ToothZones.BOTTOM], additionalInformation: "Test treatment" };
    ...
    it("create should return an observable", () => {
      expect(service.create(mockToothTreatment) instanceof Observable).toBe(true);
    });
