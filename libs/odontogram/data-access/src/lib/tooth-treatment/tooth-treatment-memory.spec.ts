...
function serviceFactory() {
  TestBed.configureTestingModule({
    imports: [],
    providers: [...ODONTOGRAM_DATA_ACCESS_PROVIDERS, ToothTreatmentMemory],
  });
...
describe('ToothTreatmentMemory', () => {
  ...
  const mockTreatment: Optional<ToothTreatment, 'id'> = {
    teeth: ['11'],
    groupTeeth: false,
  ...
  it('should apply filter by odontogram', async () => {
    ...
    const data = await firstValueFrom(
      service.getList({ odontogram: odontogramIds })
    );
    expect(Array.isArray(data)).toBe(true);
    expect(
      data.every((item) => odontogramIds.includes(item.odontogram as string))
    ).toBe(true);
    expect(
      Array.from(currentServiceData.values()).filter((item) =>
        odontogramIds.includes(item.odontogram as string)
      )
    ).toEqual(data);
  });
  ...
  it('should update an existing item', async () => {
    ...
    expect(data.additionalInformation).toEqual(
      updatedItem.additionalInformation
    );
    expect(
      currentServiceData.get(updatedItem.id)?.additionalInformation
    ).toEqual(updatedItem.additionalInformation);
  });
  ...
  it('should return error if not found on update', async () => {
    try {
      await firstValueFrom(
        service.update({
          id: '161616',
          additionalInformation: 'Non Existent Item',
        })
      );
      fail('Should have failed');
    } catch (error) {
