...
.DESCRIPTION
  ...
  damoclesSword 4203, landingV2 4204, velista 4205, luna-shopper-admin 4206. It is
  the developer's own, so a
  ...
  apps/shell/.env, apps/velista/.env, apps/luna-shopper-admin/.env, and
  tools/dev/.run (logs and pids). Nx loads
...
# office, served from its own origin (its plan 0001), so `-Up -Apps luna-shopper-admin`
# starts it alone, without the shell or any remote.
$appOrder = @('shell', 'odontogram', 'damoclesSword', 'landingV2', 'velista', 'luna-shopper-admin')
...
function Write-SlotConfig([int]$slot, [int]$backendSlot) {
  ...
  $adminPort = Get-PortFor 'luna-shopper-admin' $slot
...
NG_LUNA_SHOPPER_ADMIN_PORT=$adminPort
NG_SHELL_URL=http://localhost:$shellPort
NG_VELISTA_URL=http://localhost:$velistaPort
NG_LUNA_SHOPPER_ADMIN_URL=http://localhost:$adminPort
...
"@
  ...
  # velista and luna-shopper-admin are the two front ends that talk to a backend, so
  # they are the two whose slot has a second half. The pair defaults to the matching
  # luna slot, which is what makes "slot 2" one idea rather than two.
...
"@
  ...
  Write-EnvFile (Join-Path $root 'apps/luna-shopper-admin/.env') @"
# Generated by ng-slot.ps1 (slot $slot, luna-shopper slot $backendSlot). Git ignored.
...
"@
  ...
  Write-Host "  admin         http://localhost:$adminPort   (own origin, not a remote)"
...
function Invoke-List {
  ...
  Write-Host ""
  Write-Host ('  {0,-4} {1,-7} {2,-40} {3}' -f 'SLOT', 'STATE', 'PORTS  shell/odtg/dsword/landing/velista/admin', 'CLAIMED BY')
  ...
  for ($slot = 0; $slot -le $maxSlot; $slot++) {
    ...
    $portsCol = '{0} {1} {2} {3} {4}' -f (Get-PortFor 'shell' $slot), (Get-PortFor 'odontogram' $slot),
      (Get-PortFor 'damoclesSword' $slot), (Get-PortFor 'landingV2' $slot), (Get-PortFor 'velista' $slot), (Get-PortFor 'luna-shopper-admin' $slot)
