...
self-contained, v2 gets its own scope. Libraries to generate in `0002`:
...
- `@portfolio/landing-v2/feature-portfolio`, `.../feature-odontogram`,
  `.../feature-damocles` — the three project detail pages (`0005`).
...
> and retire v1, say so and `0002`/`0003` collapse onto the existing libs instead.
...
`0002` instead points the **empty-path `:locale` child** at `landingV2/Routes` (where
...
(`0005`) live at **`/<locale>/projects/{portfolio,odontogram,damoclesSword}`**.
...
ones to `libs/shared/ui` and export from its `index.ts` (see `0004` checklist).
...
1. **Remove header navigation** — keep the header bar, drop the nav links. → `0004`.
2. **Hero info table built dynamically** from a data-access class (structural +
   translations). → `0003` (data), `0004` (render).
3. **Remove the `03 / 03` counter** on the Projects title. → `0004`.
4. **Projects built dynamically** with **visual config** so some span more columns
   than others (`columnSpan`, `featured`). → `0003` (model+data), `0004` (grid).
5. **Add damoclesSword** to the projects list (real info; VR studio). → `0003`.
6. **Footer year set dynamically** (`new Date().getFullYear()`). → `0004`.
7. **Portfolio image proposal** (no screenshot → no self-reference loop). → `0003` §
   "Portfolio visual".
8. **e2e: no scroll / no element overflows the viewport**, 320→3840. → `0006`.
...
|------|-------|-----------|
| `0002` | Scaffold libs + route landingV2 at the locale root + empty remote-entry | app (done) |
| `0003` | data-access: projects (+damocles, visual config) & hero info-table | 0002 |
| `0004` | Landing page UI (header no-nav, dynamic hero/table/grid, footer year) | 0002, 0003 |
| `0005` | Detail pages: Portfolio, Odontogram, damoclesSword | 0002, 0003 |
| `0006` | Playwright e2e: no horizontal/vertical overflow across viewports | 0004, 0005 |
...
- **D-portfolio-image:** Portfolio uses a generated **Nx module-federation graph**
  ...
  shipped as a normal `image` asset (`0003` A.3). No self-referential screenshot.
- **D-hero-copy:** include **all** of: the "Available for work" badge, the mono role
  ...
  (Focus / Stack / Also / Based) — see `0003` B.2 and `0004`.
