...
- **Assets handling** — deep-only, kept short. The core point: **assets live with the
  libraries** that own them (i18n JSON, icons, images), and **each app bundles its assets
  at build time** (the workspace is configured to do so). Do **not** mention the backend /
  API path (there is no API today). Optional supporting detail if it reads well, drawn
  from the existing answers and the code: SVG icons imported as raw strings (`?raw`) and
  inlined via `DomSanitizer` as standalone components rather than inline `<svg>`; and the
  module-federation note that a remote's CSS `url()` assets resolve against the shell's
  origin. Daniel will polish this section's copy later.

These three deep-only sections are **confirmed** (2026-08-20).
...
punctuation. The **Assets handling** section has no dedicated case-study answer; its copy
is drawn from the two facts Daniel gave (assets co-located with their libraries; bundled
into each app at build) plus the asset-related detail already present in the answers, and
Daniel polishes it afterward. Do not invent claims beyond those.
