...
//
//   nx run luna-shopper:db:restore              # newest snapshot
//   nx run luna-shopper:db:restore -- initial   # newest snapshot labelled 'initial'
//   nx run luna-shopper:db:restore -- --dir apps/luna-shopper/.snapshots/initial-...
//
...
const SNAPSHOT_ROOT = 'apps/luna-shopper/.snapshots';
  ...
  if (!dir || !fs.existsSync(dir)) {
    console.error(
      `[restore] no snapshot found` +
        ...
        `. Take one first: nx run luna-shopper:db:snapshot -- --label initial`
    );
    process.exit(1);
  }
