...
- `nx run luna-shopper-auth:seed` inserts the auth half (users and credentials).
- `nx run luna-shopper-core:seed` inserts the core half (zones, memberships, lists, lines,
  comments), referencing the auth user ids.
- `nx run luna-shopper:seed` runs auth first, then core (order matters: core references auth
...
- **Opt in: separate test databases.** An `apps/luna-shopper/<svc>/.env.test` overrides
...
- `nx run luna-shopper:db:snapshot` runs `pg_dump` of both databases to a timestamped, git
  ignored directory (for example `apps/luna-shopper/.snapshots/<label>-<timestamp>/`), with an
  optional `--label` so a snapshot can be named (`initial`, `before-e2e`, and so on).
- `nx run luna-shopper:db:restore` runs `pg_restore` of a named or latest snapshot back into
  both databases.
...
| Target | Purpose |
| --- | --- |
| `luna-shopper-auth:seed` | Insert the auth half of the demo world |
| `luna-shopper-core:seed` | Insert the core half of the demo world |
| `luna-shopper:seed` | Orchestrate auth then core |
| `luna-shopper:db:snapshot` | `pg_dump` both databases to a labelled, git ignored dir |
| `luna-shopper:db:restore` | `pg_restore` a named or latest snapshot into both databases |
...
- Per service seeder entry points under `apps/luna-shopper/<svc>/src/app/db/seed/`, reusing the
...
- An orchestrator target and the `db:snapshot` / `db:restore` scripts (a cross platform Node or
  ...
  script) under `apps/luna-shopper/tools/` or similar, wired as workspace level Nx targets.
- `apps/luna-shopper/<svc>/.env.test.example` documenting the separate test database URLs, and
...
- `nx run luna-shopper:seed` populates both databases with the demo world, idempotently, through
