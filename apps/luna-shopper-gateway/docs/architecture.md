...
models live in `apps/luna-shopper-auth/docs/data-model.md` and
`apps/luna-shopper-core/docs/data-model.md`. Source of truth is the plan set under
`apps/luna-shopper-core/plans/`.
...
flowchart TB
    ...
    subgraph edge["Public edge"]
        gateway["luna-shopper-gateway<br/>REST + Swagger<br/>verifies JWT offline"]
        realtime["luna-shopper-realtime<br/>WebSocket + SSE<br/>socket.io"]
    end

    broker(["NATS + JetStream<br/>request/reply + durable events"])

    subgraph domain["Domain services"]
        auth["luna-shopper-auth<br/>identity, tokens, email"]
        core["luna-shopper-core<br/>zones, lists, merges"]
    end
    ...
    catalog["luna-shopper-catalog<br/>owner curated products, later"]:::future
...
- All public routes are URL versioned by major version, per controller independently
  (`/v1/...`), and documented in Swagger on the gateway (plan 0004).
