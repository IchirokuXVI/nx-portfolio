...
import { AdminHarvestLeafletsController } from './harvest.controller';
import { bodyParserProblems, jsonBodyParsers } from './leaflet-body';

/**
 * The leaflet upload over real HTTP (plan 0081, section 7).
 *
...
function leaflet(patch: Partial<LeafletDocument> = {}): LeafletDocument {
  return {
    schema_version: '1.0',
    source: { file: 'leaflet.pdf', sha256: 'd'.repeat(64), page_count: 1 },
    retailer: {
      name: 'El Jamon',
      chain_id: 'el-jamon',
      country: 'ES',
...
async function boot() {
  ...
  const moduleRef = await Test.createTestingModule({
    controllers: [AdminHarvestLeafletsController],
    providers: [
...
function post(origin: string, body: unknown): Promise<Response> {
  return fetch(`${origin}/v1/admin/harvest/leaflets`, {
    method: 'POST',
...
describe('POST /v1/admin/harvest/leaflets', () => {
  ...
  it('accepts a document larger than the default body limit', async () => {
    // The whole reason the route has its own parser. `raw_text` pads the
    // document past 1 KB, which every other route on this gateway refuses.
    const padded = leaflet();
    padded.offers[0].raw_text = Array.from(
      { length: 80 },
  ...
  it('refuses a document over its own limit, naming the number', async () => {
    const huge = leaflet();
    huge.offers[0].raw_text = Array.from({ length: 400 }, () => 'a'.repeat(64));
    expect(JSON.stringify(huge).length).toBeGreaterThan(LEAFLET_CAP);
  ...
  it('names the offer and the path when the document fails the schema', async () => {
    const broken = leaflet();
    (broken.offers[0].pricing as { basis: string }).basis = 'per-fortnight';
    ...
    expect(body.errors.join(' ')).toContain('/offers/0/pricing/basis');
    // The tile id, so the upload screen can highlight the row rather than an
  ...
  it('refuses a schema version this backend cannot read', async () => {
    const response = await post(context.origin, {
      supermarketId: SUPERMARKET,
      priceScopeId: SCOPE,
      document: { ...leaflet(), schema_version: '9.9' },
    });
