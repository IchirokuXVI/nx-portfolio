...
async function bootstrap() {
  ...
  // the surface (plan 0081, section 7). Nest's built in parser is one global
  // `express.json()` at a limit the app cannot vary per route, and its default
  // is 100 KB, so every real leaflet would be refused with a bare 413. The
  // parsers are mounted by hand below instead, largest path first.
  ...
  // The leaflet route's own limit, then the default for everything else, then
  ...
  const bodyLimits = {
    leafletMaxBytes: config.leafletMaxBytes,
