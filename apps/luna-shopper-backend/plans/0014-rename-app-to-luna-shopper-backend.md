...
Today the backend occupies the bare name `luna-shopper-backend`: the directory
`apps/luna-shopper-backend/`, the umbrella Nx project `luna-shopper-backend`, the Helm value key
`lunaShopperBackend`, and the Helm template folder `k8s/helm/templates/luna-shopper-backend/`.
When a Luna Shopper **frontend** app arrives it will want that same bare name, and
`nx run luna-shopper-backend:seed` will stop being self explanatory.
...
Not everything that contains the string `luna-shopper-backend` should become
...
| Thing | From | To |
| --- | --- | --- |
| App directory | `apps/luna-shopper-backend/` | `apps/luna-shopper-backend/` |
| Umbrella Nx project | `luna-shopper-backend` | `luna-shopper-backend` |
| Shared dev env file | `.env.luna-shopper-backend` | `.env.luna-shopper-backend` |
| Helm template folder | `k8s/helm/templates/luna-shopper-backend/` | `k8s/helm/templates/luna-shopper-backend/` |
| Helm secrets doc | `k8s/helm/luna-shopper-backend-secrets.md` | `k8s/helm/luna-shopper-backend-secrets.md` |

That alone removes the collision: the bare `luna-shopper-backend` name and the bare
`apps/luna-shopper-backend/` path are both freed for the frontend.

### Tier 2, optional consistency (recommend: **do it in the same pass**)

| Thing | From | To |
| --- | --- | --- |
| Helm values key | `lunaShopperBackend` | `lunaShopperBackendBackend` |
| Helm helper define | `lunaShopperBackend.env` | `lunaShopperBackendBackend.env` |
| Compose project name default | `luna-shopper-backend` | `luna-shopper-backend` |
...
**Service project names** (`luna-shopper-backend-auth`, `-core`, `-catalog`, `-gateway`,
`-realtime`) and their **image names** (`nx-portfolio/luna-shopper-backend-auth`, and so
on) are deliberately **left alone**. A frontend will never own a project called
`luna-shopper-backend-auth`, so there is no ambiguity to fix, and renaming them would
...
191 tracked files contain `luna-shopper-backend` or `lunaShopperBackend`, 694 occurrences
...
| --- | --- | --- | --- |
| `apps/luna-shopper-backend/**` | 448 | 135 | 313 |
| `k8s/**` | 171 | 0 | 171 |
...
majority of those are the literal path prefix `apps/luna-shopper-backend/`, which is
...
| File | Refs | What |
| --- | --- | --- |
| `.gitignore` | 9 | ignores `apps/luna-shopper-backend/.env.luna-shopper-backend`, `apps/luna-shopper-backend/*/.env`, `apps/luna-shopper-backend/*/.env.test`, `apps/luna-shopper-backend/.snapshots/`, `apps/luna-shopper-backend/secrets/*` plus the `!.../secrets/.gitkeep` negation, and `k8s/e2e/luna-shopper-backend/.env.slot` (this last one is already correct) |
| `.vscode/launch.json` | 12 | four debug configs: `nx serve luna-shopper-backend-<svc>` plus `apps/luna-shopper-backend/<svc>/dist/**` source map globs |
...
### 3.2 `apps/luna-shopper-backend/` umbrella

| File | Refs | What |
| --- | --- | --- |
| `project.json` | 6 | `"name": "luna-shopper-backend"`, `sourceRoot`, the `// ` doc comment, three `node apps/luna-shopper-backend/tools/db/*.js` commands |
| `.env.luna-shopper-backend.example` | 9 | its own filename in the copy instructions, the `apps/luna-shopper-backend/secrets/jwt.*` openssl commands, per service `.env` paths |
| `docs/testing-strategy.md` | 13 | paths and `nx run luna-shopper-backend:*` invocations |
| `tools/README.md` | 7 | paths and target names |
| `tools/db/env.js` | 4 | hardcoded `envDir` for auth/core/catalog plus `apps/luna-shopper-backend/.env.luna-shopper-backend` |
...
| File | Refs | What |
| --- | --- | --- |
| `project.json` | 17 / 17 / 16 / 12 / 12 | `sourceRoot`, `outputs` `dist/apps/luna-shopper-backend-<svc>`, build `cwd: apps/luna-shopper-backend/<svc>`, `imageName`, `jestConfig` paths, and the `migration:run|revert|generate` plus `seed` commands with full `apps/luna-shopper-backend/<svc>/src/app/db/...` paths |
| `webpack.config.js` | 1 each | `output.path` = `dist/apps/luna-shopper-backend-<svc>` (project name, Tier 2 only) |
...
| `.env.example` | 6 / 3 / 3 / 4 / 3 | `apps/luna-shopper-backend/secrets/jwt.*` paths and copy instructions |
| `.env.test.example` | 4 / 2 / 2 | same |
| `src/main.ts` | 2 each | startup log line carrying the service name (Tier 2 only) |
| `src/app/app.module.ts` | 4 / 4 / 4 / 5 / 5 | `envFilePath: ['apps/luna-shopper-backend/<svc>/.env', 'apps/luna-shopper-backend/.env.luna-shopper-backend']` and `PlatformModule.forRoot({ serviceName })` |
...
| `src/support/db.ts` | 4 | `apps/luna-shopper-backend/.snapshots/.e2e-restore-target`, `apps/luna-shopper-backend/tools/db/<script>` and a `nx run luna-shopper-backend:*` comment |
...
| File | Refs | What |
| --- | --- | --- |
| `k8s/helm/values.yaml` | 32 | the `lunaShopperBackend:` key (Tier 2), five production plus five staging service entries with `name: luna-shopper-backend-<svc>` and `image: ghcr.io/.../luna-shopper-backend-<svc>` (Tier 2), `natsUrl: nats://luna-shopper-backend-nats:4222`, three postgres entries `luna-shopper-backend-<svc>-db` with `secret: luna-shopper-backend-<svc>-db-secret`, plus prose |
| `k8s/helm/templates/luna-shopper-backend/*.tpl` (8 files) | 42 total | folder rename plus every `.Values.lunaShopperBackend` reference and the `lunaShopperBackend.env` define (`nats.yaml.tpl` 13, `deployment` 7, `migration-job` 7, `postgres` 5, `configmap`/`pdb`/`service` 3 each, `_env.tpl` 1) |
| `k8s/helm/templates/reverse-proxy/_nginx.conf.tpl` | 2 | `.Values.lunaShopperBackend.enabled` and `.services` |
| `k8s/helm/templates/reverse-proxy/deployment.yaml.tpl` | 2 | same |
| `k8s/helm/luna-shopper-backend-secrets.md` | 18 | filename, `lunaShopperBackend.*` value paths, secret names |
| `k8s/e2e/luna-shopper-backend/compose.yml` | 6 | `name: ${COMPOSE_PROJECT_NAME:-luna-shopper-backend}` (Tier 2), plus prose and `nx serve luna-shopper-backend-<svc>` examples, and a stale `cp .env.luna-shopper-backend.example .env` line |
...
`apps/luna-shopper-backend/<svc>/...` paths. Only those **prose path mentions** need
updating; the project names and the aliases stay.

## 4. Why this is cheap right now

`luna-shopper-backend` has **never reached `main`**. Verified: `git ls-tree origin/main apps/`
has no luna entry, and `lunaShopperBackend.enabled` is `false` in `values.yaml`.
Consequences:

- **No images exist in `ghcr.io/ichirokuxvi/nx-portfolio/luna-shopper-backend-*`**, so an
...
default compose project name `luna-shopper-backend` names the containers, network and
...
1. **Move directories with `git mv`** (preserves history):
   - `git mv apps/luna-shopper-backend apps/luna-shopper-backend`
   - `git mv k8s/helm/templates/luna-shopper-backend k8s/helm/templates/luna-shopper-backend`
   - `git mv k8s/helm/luna-shopper-backend-secrets.md k8s/helm/luna-shopper-backend-secrets.md`
   - `git mv apps/luna-shopper-backend/.env.luna-shopper-backend.example apps/luna-shopper-backend/.env.luna-shopper-backend.example`
2. **Rewrite the path prefix everywhere.** Order matters, longest first, so the
   ...
   1. `apps/luna-shopper-backend-e2e` and `k8s/e2e/luna-shopper-backend` are
      already correct. Protect them by rewriting `apps/luna-shopper-backend/` (with the
      trailing slash) rather than the bare `apps/luna-shopper-backend`.
   2. `apps/luna-shopper-backend/` becomes `apps/luna-shopper-backend/`
   3. `.env.luna-shopper-backend` becomes `.env.luna-shopper-backend`
   4. `templates/luna-shopper-backend/` becomes `templates/luna-shopper-backend/`
   5. `luna-shopper-backend-secrets.md` becomes `luna-shopper-backend-secrets.md`
   6. Bare `luna-shopper-backend` used as an **Nx target invocation**: `nx run luna-shopper-backend:seed`,
      `:db:snapshot`, `:db:restore` become `luna-shopper-backend:...`. Grep for
      `luna-shopper-backend:` specifically, and do **not** touch `luna-shopper/contracts`,
...
4. **Tier 2 (if taken)**: `lunaShopperBackend` becomes `lunaShopperBackendBackend` across
   `k8s/helm/**` (values, all templates, the reverse-proxy templates, the secrets
   doc), and `COMPOSE_PROJECT_NAME:-luna-shopper-backend` becomes
...
5. **Hand check** the files a blind substitution gets wrong:
   ...
   - `k8s/e2e/luna-shopper-backend/compose.yml` line 13, which already tells you
     ...
     the current `.env.luna-shopper-backend` scheme and should be corrected while here
...
one literal string, `apps/luna-shopper-backend/`, replaced by one other literal string.
...
- **Two similar names in flight.** `apps/luna-shopper-backend/` must be rewritten while
...
- **The Helm chart is text templates.** `helm lint` and `helm template` catch a
  dangling `.Values.lunaShopperBackend` only when `enabled` is true, so lint with
  `--set lunaShopperBackendBackend.enabled=true` or the rename goes unverified.
- **The `.snapshots/` directory** is git ignored and lives at
  `apps/luna-shopper-backend/.snapshots/`. Any local snapshots must be moved by hand or
...
- **If Tier 2 grows into service renames** (`luna-shopper-backend-auth` becoming
  ...
  names, `nats://luna-shopper-backend-nats`, 4 `.vscode` debug configs, the e2e
...
git grep -n 'apps/luna-shopper-backend/'            # expect: nothing
git grep -n 'luna-shopper-backend-backend'  # expect: nothing
git grep -n '\.env\.luna-shopper-backend\b'         # expect: nothing
git grep -n 'templates/luna-shopper-backend/'       # expect: nothing
git grep -n '\.Values\.lunaShopperBackend\b'       # expect: nothing (if Tier 2 taken)
...
npx nx run-many -t lint test build -p luna-shopper-backend-auth,luna-shopper-backend-core,luna-shopper-backend-catalog,luna-shopper-backend-gateway,luna-shopper-backend-realtime,luna-shopper/contracts,luna-shopper/platform,luna-shopper/test-fixtures
...
helm lint k8s/helm --set lunaShopperBackendBackend.enabled=true
helm template k8s/helm --set lunaShopperBackendBackend.enabled=true > /dev/null
...
- Once this lands, `apps/luna-shopper-backend/` and the project name `luna-shopper-backend` are
  free. A frontend should claim `luna-shopper-backend-frontend` (or, if it becomes a
  module federation remote, the remote name `lunaShopperBackend`, following the existing
