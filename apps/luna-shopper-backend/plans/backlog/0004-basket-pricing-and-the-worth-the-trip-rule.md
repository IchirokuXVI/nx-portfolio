...
it needs exist. Given a generated list (0049) and a set of price scopes (0048),
...
scope aware search) and 0049 (the generated list this writes its answer into). Nothing
...
- Anything else is a suggestion the user confirms, on the line, once, after which the `itemId` or
  ...
  0049 section 5, since it changes shared data) so the next run does not ask again.
...
For each priced line, over the scope set from 0048:
...
1. **Candidate stores.** The scopes from 0048, each carrying its locations.
...
3. **The baseline.** The single store with the lowest total for the lines it covers, tie broken
   by coverage first, then by the user's preference order (0048, `position` on the postal
...
0048 is **per extra stop, not per line and not per basket**.
...
Pricing writes onto the generated list from 0049 rather than into a new structure:
...
Everything is a **snapshot**, consistent with 0049 section 4. Prices move; a basket
