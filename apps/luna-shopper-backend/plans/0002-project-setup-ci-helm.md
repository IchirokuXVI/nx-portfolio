...
`apps/luna-shopper-backend-<svc>/src/Dockerfile`, multi stage:

1. Build stage `FROM` the repo `builder` image, run `npx nx build luna-shopper-backend-<svc>`.
...
mirroring the existing apps: `imageName` of `nx-portfolio/luna-shopper-backend-<svc>`, and
...
- `nx run luna-shopper-backend-<svc>:build:docker` produces a runnable image for each service.
