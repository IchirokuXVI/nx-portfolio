...
> `apps/luna-shopper-backend-*/plans/` or a shared `plans/` under the Luna Shopper apps), following
...
service, `luna-shopper-backend-catalog` (owner curated products and supermarkets), is added last in plan
...
2. Generate the apps under `apps/` as NestJS applications (`@nx/nest:application`) with jest
   and eslint targets:
   - `luna-shopper-backend-gateway`
   - `luna-shopper-backend-realtime`
...
- The four `apps/luna-shopper-backend-*` services exist and boot; `libs/luna-shopper/contracts`
  exists and is importable.
- `nx run-many --target=build --projects=luna-shopper-backend-*` is green.
