...
import { AUTH_MIGRATIONS } from './index';
...
describe('AUTH_MIGRATIONS', () => {
  ...
  it('lists every migration file in the directory', () => {
    ...
    const listed = AUTH_MIGRATIONS.map((m) => m.name).sort();
    const onDisk = files
      .map((f) => {
  ...
  it('is ordered by timestamp', () => {
    ...
    const timestamps = AUTH_MIGRATIONS.map((m) => {
  ...
  it('is not empty', () => {
    ...
    expect(AUTH_MIGRATIONS.length).toBeGreaterThan(0);
  });
});
