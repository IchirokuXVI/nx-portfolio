...
`luna-shopper-backend` Nx project. Two independent mechanisms that share nothing but the
fixture ids:
...
- **Seed (a population concern).** Inserts the canonical demo world from
  ...
  ```sh
  nx run luna-shopper-backend:seed                 # auth, then catalog, then core
  LUNA_ENV=test nx run luna-shopper-backend:seed   # target the opt-in test databases
  ```
...
- **Snapshot / restore (a backup concern).** `pg_dump` / `pg_restore` of the
  ...
  nx run luna-shopper-backend:db:restore  -- initial     # restoring 'initial' IS the reset
  ```
...
Snapshots are written to `apps/luna-shopper-backend/.snapshots/` (git-ignored).
