...
import { BasketParticipant } from '../entities';
import { CoreEventsPublisher } from '../events/core-events.publisher';
import {
  COMMON_GROUPS_SQL,
  type CommonGroupsRow,
} from './basket-members.sql';
import { toParticipantView } from './basket-sharing.mappers';
 ...
 */
@Injectable()
export class BasketMembersService {
  constructor(
  ...
  nameFor(
    ...
    return trimmed
      ? trimmed.slice(0, BASKET_SHARING_LIMITS.displayNameMaxLength)
      : null;
  }
   ...
   */
  async lock(manager: EntityManager, basketId: string): Promise<void> {
    await manager.query(
  ...
  async checkRoom(
    ...
    const total = await manager.count(BasketParticipant, {
      where: { basketId, ...liveParticipantWhere() },
    });
    if (total >= BASKET_SHARING_LIMITS.maxParticipants) {
  ...
  async invite(
    ...
    const existing = await manager.findOne(BasketParticipant, {
      where: { basketId: args.basketId, userId: args.userId },
    });
    ...
    await this.checkRoom(manager, args.basketId);
    ...
    const created = await manager.save(
      manager.create(BasketParticipant, {
        ...
        // `ck_basket_participants_expiry` holds against `invitedAt`.
        expiresAt: null,
      })
    );
  ...
  liveRegistered(basketId: string): Promise<BasketParticipant[]> {
    return this.dataSource.getRepository(BasketParticipant).find({
      where: {
        basketId,
        kind: ParticipantKind.REGISTERED,
  ...
  announceEnded(
    ...
    if (participant.kind === ParticipantKind.REGISTERED && participant.userId) {
      this.announceUnshared(basketId, participant.userId);
    }
  }

  /** One person is no longer on a basket, told to their own sessions. */
  announceUnshared(basketId: string, userId: string): void {
    this.announceAccess(
  ...
  private announceAccess(
    ...
    const payload: BasketAccessEvent = { basketId };
