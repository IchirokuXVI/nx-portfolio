 ...
 *
 * The cutoff is `basket.claimWindowMs`, the number `LINE_CLAIMS_SQL`
 ...
 * own room** since plan 0139, and {@link BasketService.update} is where
 ...
 */
@Injectable()
export class BasketSweepService
  implements OnApplicationBootstrap, OnApplicationShutdown
{
  ...
  private async tick(): Promise<void> {
    ...
    try {
      ...
      this.logger.error({ err }, 'basket sweep failed');
    } finally {
      this.running = false;
    }
  }
  ...
  async sweep(): Promise<number> {
    ...
    for (const list of stale) {
      try {
        ...
        this.logger.error(
          { err, basketId: list.id },
          'basket sweep could not finish a basket'
        );
      }
    }
    if (finished > 0) {
      this.logger.log(
        { count: finished },
        'basket sweep finished baskets past the claim window'
      );
    }
    return finished;
  }
