...
import type { BasketMembersService } from './basket-members.service';
import { BasketService } from './basket.service';
import { WRITABLE_LISTS_SQL } from './basket.sql';
 ...
 *
 * **The run composes nothing since plan 0136.** It writes a `baskets`
...
function build(options: {
  ...
  const listRepo = {
    ...
    save: async (row: Partial<Basket>) => row,
  ...
  } as unknown as BasketMembersService;
...
describe('the generation run', () => {
  ...
  it('stores no product set and no pick, which the read composes instead', async () => {
    ...
    // a free text line with no pick and no options". `basket_line_options`
  ...
  it('starts a basket OPEN and GENERATED, and tells only the owner', async () => {
    ...
    expect(written.lists[0].status).toBe(BasketStatus.OPEN);
    ...
    expect(events).toEqual([
      {
        event: RealtimeEvent.BasketCreated,
        userIds: [OWNER],
      },
    ]);
  });
...
/**
 * The one zone event a basket emits (plan 0052).
 *
 * Plan 0050 section 8 said baskets never emit zone events, and this is
...
describe('a basket tells the lists it touches (plan 0122, section 6)', () => {
  ...
  it('tells them when the basket is deleted, finished or not', async () => {
    const w = build({
      existing: { ...existing, status: BasketStatus.FINISHED },
      covering: COVERING,
    });
