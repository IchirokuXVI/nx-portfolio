...
describe('BasketsModule wiring', () => {
  ...
  it('imports the three modules its services are constructed from', () => {
    const imports = importsOf(BasketsModule);
  ...
  it('provides the read and the five writes on a row', () => {
    ...
    const providers = providersOf(BasketsModule);
    ...
    // read rather than in `baskets/`, where it used to be: the run asks
    // it nothing any more.
  ...
  it('exports the basket read, which the history and the back office count through', () => {
    ...
    // the screen.
    expect(exportsOf(BasketsModule)).toContain(BasketReadService);
  });
  ...
  it('provides the sweep, which nothing injects and Nest alone would start', () => {
    ...
    expect(providersOf(BasketsModule)).toContain(
      BasketSweepService
    );
  });
  ...
  it('exports the basket service, which account deletion reaches for', () => {
    ...
    expect(exportsOf(BasketsModule)).toContain(BasketService);
  });
});
