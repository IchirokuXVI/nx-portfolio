 ...
 *
 * Raw SQL for the reason `basket-sharing.sql.ts` gives, and every
...
export const SHARED_BASKETS_SQL = `
  ...
  WHERE p."userId" = $1
    ...
    AND (
      $2::uuid IS NULL
      OR (COALESCE(p."invitedAt", p."joinedAt"), p."basketId") < (
        SELECT COALESCE(b."invitedAt", b."joinedAt"), b."basketId"
        FROM "basket_participants" b
        WHERE b."basketId" = $2 AND b."userId" = $1
      )
    )
  ORDER BY "sharedAt" DESC, p."basketId" DESC
  LIMIT $3
`;
