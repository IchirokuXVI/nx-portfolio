...
function build(rows: Basket[]): Harness {
...
describe('what the permanent basket refuses (section 2)', () => {
  it('refuses a name, and says which field it was about', async () => {
    ...
    const failed = await service
      .update({ userId: OWNER, basketId: LIVE, name: 'Saturday' })
      .catch((error: unknown) => error);

    // Refused here rather than by `ck_baskets_live_shape`, so the caller
  ...
  it('refuses those two and nothing else', async () => {
    ...
    const view = await service.update({ userId: OWNER, basketId: LIVE });
  ...
  it('refuses to be deleted, and leaves the row where it is', async () => {
    ...
    await expect(
      service.delete({ userId: OWNER, basketId: LIVE })
  ...
  it('announces no claim in either direction', async () => {
    ...
    await service.update({ userId: OWNER, basketId: LIVE });
...
describe('the sweep leaves the permanent basket alone (section 6)', () => {
  ...
  function sweeper(rows: Basket[]) {
    ...
    const generated = {
      update: async (req: { basketId: string }) => {
        finished.push(req.basketId);
        return {};
      },
    ...
    return {
      finished,
      service: new BasketSweepService(
        lists as never,
        generated,
  ...
  it('finishes an old trip and leaves a permanent basket of the same age', async () => {
    ...
    const w = sweeper([
      ...
      { ...liveBasket(), generatedAt: old } as Basket,
    ]);
