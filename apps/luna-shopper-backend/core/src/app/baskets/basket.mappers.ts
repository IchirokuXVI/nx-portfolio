 ...
 * line** a settlement landed on, which is what `basket_line_origins`
 ...
 */
export function toBasketHeaderView(
  row: Basket,
 ...
 */
export function toBasketHistoryView(
  row: Basket,
  counts: BasketLineCounts,
  presentCount = 0
): BasketHistoryView {
  return {
    id: row.id,
...
export interface BasketLineCounts {
  lineCount: number;
...
export const NO_GENERATED_LINE_COUNTS: BasketLineCounts = {
  lineCount: 0,
  settledLineCount: 0,
  boughtLineCount: 0,
