 ...
 *
 * The service under test is wired to a **real** {@link BasketService}
 ...
 * transaction hands out a repository for `baskets` alone
...
/** Sixty hours, which is what `BASKET_CLAIM_WINDOW` defaults to. */
...
function build(options: {
  ...
  const generated = new BasketService(
  ...
  const service = new BasketSweepService(
    lists as never,
    generated,
    logger as never,
...
describe('BasketSweepService.sweep', () => {
  it('finishes every live basket past the window, settled or not, and leaves a fresh one', async () => {
    const harness = build({
      baskets: [
        ...
        { id: 'gl-draft', status: BasketStatus.OPEN, ageMs: 61 * HOUR },
    ...
    expect(statusOf(harness, 'gl-draft')).toBe(BasketStatus.FINISHED);
    expect(statusOf(harness, 'gl-active')).toBe(BasketStatus.FINISHED);
    expect(statusOf(harness, 'gl-fresh')).toBe(BasketStatus.OPEN);
  });
  ...
  it('never touches ARCHIVED, and has nothing to add to COMPLETED', async () => {
    ...
    expect(statusOf(harness, 'gl-archived')).toBe(BasketStatus.ARCHIVED);
    expect(statusOf(harness, 'gl-done')).toBe(BasketStatus.FINISHED);
  ...
  it('caps a tick at the batch, oldest first, and the rest waits for the next one', async () => {
    ...
    await expect(harness.service.sweep()).resolves.toBe(2);
    expect(statusOf(harness, 'gl-oldest')).toBe(BasketStatus.FINISHED);
    expect(statusOf(harness, 'gl-middle')).toBe(BasketStatus.FINISHED);
    expect(statusOf(harness, 'gl-newest')).toBe(BasketStatus.OPEN);

    await expect(harness.service.sweep()).resolves.toBe(1);
    expect(statusOf(harness, 'gl-newest')).toBe(BasketStatus.FINISHED);
  ...
  it('keeps going when a basket vanished between the query and the write', async () => {
    ...
    await expect(harness.service.sweep()).resolves.toBe(1);
    expect(statusOf(harness, 'gl-here')).toBe(BasketStatus.FINISHED);
  ...
  it('draws the cutoff from the claim window, so a live basket never outlives its claim', async () => {
    ...
    await expect(harness.service.sweep()).resolves.toBe(1);
    expect(statusOf(harness, 'gl-inside')).toBe(BasketStatus.OPEN);
    expect(statusOf(harness, 'gl-past')).toBe(BasketStatus.FINISHED);
  });
});

describe('BasketSweepService lifecycle', () => {
