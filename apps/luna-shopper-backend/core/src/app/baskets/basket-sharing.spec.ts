...
import { BasketParticipant, BasketShareLink } from '../entities';
...
import { COMMON_GROUPS_SQL } from './basket-members.sql';
import {
 ...
 *
 * The repositories are faked in the style `basket-run.spec.ts`
...
interface Harness {
  service: BasketSharingService;
  links: Partial<BasketShareLink>[];
  participants: Partial<BasketParticipant>[];
...
function build(
  options: {
    ...
    loseTheLinkRace?: Partial<BasketShareLink>;
  ...
  const query = async (sql: string, params: unknown[]): Promise<unknown[]> => {
    ...
    if (sql.includes('INSERT INTO "basket_share_links"')) {
      const [basketId, secret, createdByParticipantId, ttl] = params as [
        string,
        string,
        string,
        number,
    ...
    if (sql.includes('UPDATE "basket_share_links"')) {
    ...
    if (sql.includes('UPDATE "basket_participants"')) {
  ...
  const participantRepo = {
    ...
    create: (data: Partial<BasketParticipant>) => ({ ...data }),
    save: async (row: Partial<BasketParticipant>) => {
    ...
    update: async (rowId: string, patch: Partial<BasketParticipant>) => {
  ...
  const manager = {
    ...
    create: (_entity: unknown, data: Partial<BasketParticipant>) => ({
      ...data,
    }),
    findOne: async (
      entity: unknown,
  ...
  const service = new BasketSharingService(
    dataSource,
    listRepo as never,
    linkRepo as never,
...
describe('a basket has zero share links or one (section 3)', () => {
  ...
  it('answers an unshared basket with no link rather than an error', async () => {
    const harness = build();
    await expect(
      harness.service.getLink({ userId: OWNER, basketId: BASKET })
    ).resolves.toEqual({});
  });
...
describe('a revoked participant is refused with no cache to wait out (section 3.3)', () => {
  ...
  it('refuses a request presenting no credential at all', async () => {
    const harness = build();
    await expect(
      harness.service.resolveParticipant({ basketId: BASKET })
    ).rejects.toBeInstanceOf(DomainException);
  });
});
...
describe('what a participant may see (plan 0136, section 3.4)', () => {
   ...
   * per list and is answered by {@link BasketSharingService.writableAmong},
