...
import type { BasketSharingService } from './basket-sharing.service';
 ...
 *
 * It asserts the **code**, `basket_finished`, and never the message. The
...
const ITEM = 'item-1';
const FINISHED = 'basket_finished';
...
function basketWith(status: BasketStatus): Basket {
  return {
    id: BASKET,
    ownerUserId: OWNER,
...
function build(status: BasketStatus): Harness {
  ...
  const sharing = {
    ...
    writableAmong: async (_userId: string, listIds: string[]) =>
      new Set(listIds),
  } as unknown as BasketSharingService;
...
const WRITES = Object.keys(build(BasketStatus.FINISHED).writes);
...
describe('a finished basket refuses every write (section 3)', () => {
  ...
  describe.each(WRITES)('%s', (name) => {
    ...
    it('gets past the status on an OPEN basket, so the refusal above was the status', async () => {
      const harness = build(BasketStatus.OPEN);
...
describe('what a finished basket still does (section 3.4)', () => {
  it('still opens for a guest who was already in it', async () => {
    ...
    expect(opened.basket.status).toBe(BasketStatus.FINISHED);
...
describe('finishing and unfinishing (section 2)', () => {
  ...
  function owned(status: BasketStatus) {
  ...
  it('unfinishes through the same PATCH, and the claims come back with it', async () => {
    const w = owned(BasketStatus.FINISHED);
    ...
    expect(view.status).toBe(BasketStatus.OPEN);
  ...
  it('answers not found to a participant who is not the owner, and moves nothing', async () => {
    const w = owned(BasketStatus.OPEN);
