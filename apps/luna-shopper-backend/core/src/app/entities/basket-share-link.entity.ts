 ...
 *
 * That invariant is a **partial unique index** over `basketId` where
 ...
 * again.
 */
@Entity({ name: 'basket_share_links' })
@Index('uq_basket_share_links_live', ['basketId'], {
  unique: true,
  where: '"revokedAt" IS NULL',
})
export class BasketShareLink extends BaseEntity {
  @Column({ type: 'uuid' })
  basketId!: string;
   ...
   */
  @Index('uq_basket_share_links_secret', { unique: true })
