...
interface Harness {
  ...
  events: { event: RealtimeEvent; generatedListId?: string; userIds?: string[] }[];
}

function build(expired: Partial<GeneratedListParticipant>[] = []): Harness {
  ...
  const dataSource = {
    query: async (sql: string, params: unknown[] = []) => {
      ...
      return sql.includes('generated_list_participants') ? expired : [];
    },
...
): Partial<GeneratedListParticipant> {
  return {
    id,
...
describe('the sweep ends expired access (section 7)', () => {
  ...
  it('announces one participantLeft per row, which is the whole eviction', async () => {
    ...
    expect(
      harness.events.filter(
        (e) => e.event === RealtimeEvent.GeneratedListParticipantLeft
      )
    ).toHaveLength(2);
  });
  ...
  it('tells a registered visitor’s own sessions the basket is no longer theirs', async () => {
    ...
    expect(harness.events).toContainEqual({
      event: RealtimeEvent.GeneratedListUnshared,
      userIds: ['u-1'],
    });
  });
  ...
  it('revokes expired links in the same tick, and announces nothing for them', async () => {
    ...
    const links = harness.statements.find((s) =>
      s.sql.includes('generated_list_share_links')
    );
