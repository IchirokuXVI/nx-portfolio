 ...
 *
 * `announceEnded` emits `generatedList.participantLeft` to the basket room.
 ...
 * `generatedList.unshared` and drop the basket from their shared listing.
  ...
  implements OnApplicationBootstrap, OnApplicationShutdown
{
  ...
  async sweep(): Promise<number> {
    ...
    const expired: GeneratedListParticipant[] =
    ...
    for (const participant of expired) {
      this.members.announceEnded(participant.generatedListId, participant);
    }
