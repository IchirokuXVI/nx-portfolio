...
export interface FakeAudit {
  service: CoreAuditService;
  recorded: RecordedChange[];
}

/**
 * A {@link CoreAuditService} for the service specs, which construct their subject
 * by hand with fake repositories rather than through a Nest module.
 *
 ...
 * built, so a spec that asserted on `memberships.save` keeps asserting on exactly
 * that and says nothing new about the trail. What is genuinely transactional is
 * proven in `core-audit.integration.spec.ts` against a real Postgres, because a
 * fake cannot prove a rollback.
 *
...
export function fakeAudit(
  ...
  } as unknown as CoreAuditService;
