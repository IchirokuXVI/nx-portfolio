...
Entity relationship and class diagrams for `luna-shopper-backend-core`: zones, membership, merges, and
...
The catalog is its own service (`luna-shopper-backend-catalog`, plan 0012) with its own database, built
last and owner curated. It is shown here because `ListLine.itemId` optionally references it. When
the catalog app is scaffolded this diagram moves to `apps/luna-shopper-backend/catalog/docs/`.
