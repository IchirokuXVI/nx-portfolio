...
import { CATALOG_MIGRATIONS } from './index';
...
describe('CATALOG_MIGRATIONS', () => {
  ...
  it('lists every migration file in the directory', () => {
    ...
    const listed = CATALOG_MIGRATIONS.map((m) => m.name).sort();
    const onDisk = files
      .map((f) => {
  ...
  it('is ordered by timestamp', () => {
    ...
    const timestamps = CATALOG_MIGRATIONS.map((m) => {
  ...
  it('is not empty', () => {
    ...
    expect(CATALOG_MIGRATIONS.length).toBeGreaterThan(0);
  });
});
