...
SERVICES=(gateway realtime auth core catalog harvester)
...
write_config() {
  ...
  local GATEWAY_PORT REALTIME_PORT AUTH_PORT CORE_PORT CATALOG_PORT HARVESTER_PORT
...
Configured this worktree for Luna Shopper slot ${slot}.
  ...
  gateway ${GATEWAY_PORT}   realtime ${REALTIME_PORT}   auth ${AUTH_PORT}   core ${CORE_PORT}   catalog ${CATALOG_PORT}   harvester ${HARVESTER_PORT}
...
list() {
  ...
  echo "  SERVICES  gateway, realtime, auth, core, catalog, harvester"
