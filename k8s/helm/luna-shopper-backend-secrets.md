...
Per environment application secret — `luna-shopper-backend-secrets-production` and, if
staging is enabled, `luna-shopper-backend-secrets-staging`:
...
`luna-shopper-backend-core-db-secret`, and `luna-shopper-backend-catalog-db-secret`:
...
`luna-shopper-backend-auth-db-secret` (same for core).
...
```sh
kubectl -n nx-portfolio create secret generic luna-shopper-backend-auth-db-secret \
  --from-literal=POSTGRES_PASSWORD="$(openssl rand -base64 24)"
kubectl -n nx-portfolio create secret generic luna-shopper-backend-core-db-secret \
  --from-literal=POSTGRES_PASSWORD="$(openssl rand -base64 24)"
kubectl -n nx-portfolio create secret generic luna-shopper-backend-catalog-db-secret \
  --from-literal=POSTGRES_PASSWORD="$(openssl rand -base64 24)"
```
...
```sh
kubectl -n nx-portfolio create secret generic luna-shopper-backend-secrets-production \
  --from-literal=AUTH_DB_URL="postgres://luna_auth:PW@luna-shopper-backend-auth-db:5432/luna_auth" \
  --from-literal=CORE_DB_URL="postgres://luna_core:PW@luna-shopper-backend-core-db:5432/luna_core" \
  --from-literal=CATALOG_DB_URL="postgres://luna_catalog:PW@luna-shopper-backend-catalog-db:5432/luna_catalog" \
...
names (e.g. `luna_auth_staging`) in `luna-shopper-backend-secrets-staging` and creating
